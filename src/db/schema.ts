...
export const observations = mysqlTable(
  "observations",
  {
    ...
    observationTagsArray: json("observation_tags_array"),
    observationDatetimeStart: datetime("observation_datetime_start"),
    observationDatetimeEnd: datetime("observation_datetime_end"),
    imageData: blob("image_data"),
  },
  (table) => [index("account_id_key").on(table.authorId)],
);

export const infoPageImages = mysqlTable("info_page_images", {
